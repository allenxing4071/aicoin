# 服务器连接配置模板

## 📋 概述

本文档提供服务器 SSH 连接配置的标准模板。**请勿在此文档中填写真实的连接信息。**

真实的连接信息应存放在：`docs/07-部署运维/私密配置/` 目录（该目录已被 `.gitignore` 排除）

---

## 🔑 SSH 密钥连接配置

### 1. 服务器信息模板

```
================================================================================
                        服务器连接信息
================================================================================

IP 地址: your_server_ip_here
用户名: your_username_here
私钥文件: ~/.ssh/your-server-key.pem
SSH 端口: 22

================================================================================
客户端连接命令
================================================================================

ssh -i ~/.ssh/your-server-key.pem your_username@your_server_ip

================================================================================
快速配置（3步）
================================================================================

1. 拷贝私钥文件到客户端电脑的 ~/.ssh/ 目录
2. 设置权限: chmod 600 ~/.ssh/your-server-key.pem
3. 连接: ssh -i ~/.ssh/your-server-key.pem your_username@your_server_ip

================================================================================
```

### 2. SSH Config 配置模板

创建或编辑 `~/.ssh/config` 文件：

```bash
Host your-server-alias
    HostName your_server_ip
    User your_username
    Port 22
    IdentityFile ~/.ssh/your-server-key.pem
    ServerAliveInterval 60
    ServerAliveCountMax 3
```

设置正确的权限：

```bash
chmod 600 ~/.ssh/config
```

配置后可以简化连接：

```bash
ssh your-server-alias
```

---

## 🔐 密钥生成步骤

### 在服务器端生成 PEM 格式密钥对

```bash
# 生成 RSA 4096 位密钥对
ssh-keygen -t rsa -b 4096 -f ~/.ssh/server-key -C "server-$(date +%Y%m%d)" -m PEM

# 生成的文件：
# - server-key (私钥，需要保密)
# - server-key.pub (公钥，可以公开)
```

### 配置服务器端

```bash
# 将公钥添加到授权文件
cat ~/.ssh/server-key.pub >> ~/.ssh/authorized_keys

# 设置正确的权限
chmod 700 ~/.ssh
chmod 600 ~/.ssh/authorized_keys
chmod 600 ~/.ssh/server-key
chmod 644 ~/.ssh/server-key.pub
```

### 配置客户端

```bash
# 1. 将私钥传输到客户端（使用安全方式，如 U盘）
# 2. 移动到 .ssh 目录
mkdir -p ~/.ssh
mv server-key ~/.ssh/
chmod 600 ~/.ssh/server-key

# 3. 测试连接
ssh -i ~/.ssh/server-key your_username@your_server_ip
```

---

## 📁 文件传输

### 上传文件到服务器

```bash
# 单个文件
scp -i ~/.ssh/your-server-key.pem local_file.txt your_username@your_server_ip:~/

# 整个目录
scp -i ~/.ssh/your-server-key.pem -r local_folder/ your_username@your_server_ip:~/
```

### 从服务器下载文件

```bash
# 单个文件
scp -i ~/.ssh/your-server-key.pem your_username@your_server_ip:~/remote_file.txt ./

# 整个目录
scp -i ~/.ssh/your-server-key.pem -r your_username@your_server_ip:~/remote_folder/ ./
```

---

## 🖥️ VS Code 远程开发

### 1. 安装扩展

在 VS Code 中安装 `Remote - SSH` 扩展。

### 2. 配置 SSH

确保 `~/.ssh/config` 已正确配置（见上文）。

### 3. 连接到服务器

1. 按 `F1` 或 `Cmd+Shift+P`
2. 输入 `Remote-SSH: Connect to Host`
3. 选择配置的服务器别名
4. 开始远程开发

### 4. 端口转发

如果需要访问服务器上的 Web 服务：

```bash
# 方式 1: SSH 命令行
ssh -i ~/.ssh/your-server-key.pem -L 8080:localhost:3000 your_username@your_server_ip

# 方式 2: 在 VS Code 的端口转发面板中配置
# 本地端口: 8080
# 远程端口: 3000
```

然后在浏览器访问 `http://localhost:8080`

---

## 🛡️ 安全最佳实践

### 1. 密钥权限

```bash
# 私钥必须是 600
chmod 600 ~/.ssh/your-server-key.pem

# 公钥可以是 644
chmod 644 ~/.ssh/your-server-key.pem.pub

# .ssh 目录应该是 700
chmod 700 ~/.ssh

# authorized_keys 应该是 600
chmod 600 ~/.ssh/authorized_keys
```

### 2. 服务器 SSH 配置加固

编辑 `/etc/ssh/sshd_config`：

```bash
# 启用公钥认证
PubkeyAuthentication yes

# 禁用密码登录（确保密钥登录正常后再启用）
PasswordAuthentication no

# 禁用 root 直接登录
PermitRootLogin no

# 限制登录尝试次数
MaxAuthTries 3

# 设置空闲超时
ClientAliveInterval 300
ClientAliveCountMax 2
```

重启 SSH 服务：

```bash
sudo systemctl restart sshd
```

### 3. 定期轮换密钥

建议每 3-6 个月更换一次 SSH 密钥。

### 4. 密钥备份

- 将私钥备份到安全的密码管理器
- 不要通过云盘同步
- 不要通过邮件或聊天工具传输

---

## 🔧 故障排查

### 问题 1: Permission denied (publickey)

**原因**: 私钥文件权限不正确

**解决**:
```bash
chmod 600 ~/.ssh/your-server-key.pem
```

### 问题 2: WARNING: UNPROTECTED PRIVATE KEY FILE!

**原因**: 私钥文件权限过于开放

**解决**:
```bash
chmod 600 ~/.ssh/your-server-key.pem
```

### 问题 3: 仍然要求输入密码

**可能原因**:
1. 服务器端 `PubkeyAuthentication` 未启用
2. `~/.ssh/authorized_keys` 权限不正确
3. 使用了错误的私钥

**解决**:
```bash
# 在服务器端检查
sudo grep "PubkeyAuthentication" /etc/ssh/sshd_config
chmod 600 ~/.ssh/authorized_keys
chmod 700 ~/.ssh

# 在客户端使用 -v 参数查看详细信息
ssh -v -i ~/.ssh/your-server-key.pem your_username@your_server_ip
```

### 问题 4: Host key verification failed

**原因**: 服务器的 host key 发生变化

**解决**:
```bash
# 从 known_hosts 中删除旧的记录
ssh-keygen -R your_server_ip

# 或者编辑 ~/.ssh/known_hosts 手动删除对应行
```

---

## 📖 相关文档

- [敏感信息管理规范](./00-敏感信息管理规范.md)
- [远程服务器部署手册](./06-远程服务器部署手册.md)
- [部署指南](./01-部署指南.md)

---

## ⚠️ 重要提醒

1. **不要在本文档中填写真实的连接信息**
2. **真实信息应存放在** `docs/07-部署运维/私密配置/` 目录
3. **私钥文件必须设置 600 权限**
4. **定期轮换 SSH 密钥**
5. **将私钥备份到安全位置**

---

*最后更新: 2025-11-10*  
*版本: v1.0*  
*状态: ✅ 公开模板*

