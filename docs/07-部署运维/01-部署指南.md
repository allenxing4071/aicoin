# AIcoin v2.0 Docker部署指南

## 📋 目录

1. [系统要求](#系统要求)
2. [快速开始](#快速开始)
3. [测试网部署](#测试网部署)
4. [生产环境部署](#生产环境部署)
5. [配置说明](#配置说明)
6. [常见问题](#常见问题)
7. [监控和维护](#监控和维护)

---

## 系统要求

### 硬件要求
- **CPU**: 4核心或以上
- **内存**: 8GB RAM或以上
- **存储**: 50GB可用空间
- **网络**: 稳定的互联网连接

### 软件要求
- **Docker**: 20.10或以上
- **Docker Compose**: 2.0或以上
- **操作系统**: Linux (推荐 Ubuntu 20.04+) / macOS / Windows with WSL2

### 检查Docker安装
```bash
docker --version
docker-compose --version
```

---

## 快速开始

### 1. 克隆项目
```bash
git clone <repository-url>
cd AIcoin
```

### 2. 构建Docker镜像
```bash
./scripts/build_docker.sh
```

### 3. 选择部署环境
- **测试网**: `./scripts/start_testnet.sh`
- **生产环境**: `./scripts/deploy_prod.sh`

---

## 测试网部署

### 步骤1: 配置环境变量
```bash
# 复制示例配置
cp .env.testnet.example .env.testnet

# 编辑配置文件
nano .env.testnet
```

### 步骤2: 填写必需配置
```bash
# AI API密钥
DEEPSEEK_API_KEY=sk-your-deepseek-api-key

# Hyperliquid测试网配置
HYPERLIQUID_WALLET_ADDRESS=0xYourTestnetWalletAddress
HYPERLIQUID_PRIVATE_KEY=0xYourTestnetPrivateKey
HYPERLIQUID_TESTNET=true

# 交易配置（建议先设为false观察）
TRADING_ENABLED=false
```

### 步骤3: 启动服务
```bash
./scripts/start_testnet.sh
```

### 步骤4: 验证部署
```bash
# 检查服务状态
docker-compose -f deploy/docker-compose.testnet.yml ps

# 检查后端健康
curl http://localhost:8000/health

# 检查前端
curl http://localhost:3000
```

### 步骤5: 访问系统
- **前端界面**: http://localhost:3000
- **API文档**: http://localhost:8000/docs
- **Qdrant控制台**: http://localhost:6333/dashboard

---

## 生产环境部署

### ⚠️ 重要提醒
生产环境部署前，请确保：
1. 已在测试网充分测试
2. 理解所有风险参数
3. 配置了监控和告警
4. 准备好应急预案

### 步骤1: 配置环境变量
```bash
# 复制示例配置
cp .env.prod.example .env.prod

# 编辑配置文件（使用强密码！）
nano .env.prod
```

### 步骤2: 填写生产配置
```bash
# 强密码配置
SECRET_KEY=your-super-secret-key-change-this-in-production
POSTGRES_PASSWORD=your-strong-postgres-password
REDIS_PASSWORD=your-strong-redis-password

# AI API密钥
DEEPSEEK_API_KEY=sk-your-deepseek-api-key

# Hyperliquid主网配置
HYPERLIQUID_WALLET_ADDRESS=0xYourMainnetWalletAddress
HYPERLIQUID_PRIVATE_KEY=0xYourMainnetPrivateKey
HYPERLIQUID_TESTNET=false
HYPERLIQUID_API_URL=https://api.hyperliquid.xyz

# 交易配置（建议先设为false观察）
TRADING_ENABLED=false
DECISION_INTERVAL=300
INITIAL_PERMISSION_LEVEL=L1

# 风控参数
MAX_POSITION_SIZE=10000
MAX_DAILY_LOSS=1000
MAX_DRAWDOWN=0.15
```

### 步骤3: 配置SSL证书（可选）
```bash
# 创建SSL目录
mkdir -p nginx/ssl

# 放置证书文件
cp your-cert.pem nginx/ssl/cert.pem
cp your-key.pem nginx/ssl/key.pem

# 编辑nginx配置启用HTTPS
nano nginx/nginx.conf
```

### 步骤4: 部署
```bash
./scripts/deploy_prod.sh
```

### 步骤5: 验证部署
```bash
# 检查所有服务
docker-compose -f deploy/docker-compose.prod.yml ps

# 检查健康状态
curl http://localhost/health
curl http://localhost/api/v1/status
```

---

## 配置说明

### 核心配置项

#### 数据库配置
```bash
POSTGRES_USER=aicoin
POSTGRES_PASSWORD=your-strong-password
DATABASE_URL=postgresql://aicoin:password@postgres:5432/aicoin
```

#### Redis配置
```bash
REDIS_PASSWORD=your-redis-password
REDIS_URL=redis://:password@redis:6379
```

#### AI模型配置
```bash
DEEPSEEK_API_KEY=sk-xxx        # 必需
QWEN_API_KEY=sk-xxx            # 可选
GROK_API_KEY=xai-xxx           # 可选
CLAUDE_API_KEY=sk-ant-xxx      # 可选
OPENAI_API_KEY=sk-xxx          # 可选
```

#### 交易配置
```bash
TRADING_ENABLED=false          # 是否启用真实交易
DECISION_INTERVAL=300          # 决策间隔（秒）
INITIAL_PERMISSION_LEVEL=L1    # 初始权限等级（L0-L5）
```

#### 风控配置
```bash
MAX_POSITION_SIZE=10000        # 最大持仓金额（USD）
MAX_DAILY_LOSS=1000            # 最大日亏损（USD）
MAX_DRAWDOWN=0.15              # 最大回撤比例（15%）
```

---

## 常见问题

### Q1: 端口冲突
**问题**: 启动时提示端口已被占用

**解决**:
```bash
# 检查端口占用
lsof -i :8000
lsof -i :3000

# 修改docker-compose.yml中的端口映射
ports:
  - "8001:8000"  # 改为其他端口
```

### Q2: 数据库连接失败
**问题**: Backend无法连接到PostgreSQL

**解决**:
```bash
# 检查PostgreSQL容器状态
docker-compose ps postgres

# 查看PostgreSQL日志
docker-compose logs postgres

# 重启PostgreSQL
docker-compose restart postgres
```

### Q3: 内存不足
**问题**: 容器频繁重启或OOM

**解决**:
```bash
# 限制容器内存使用
# 在docker-compose.yml中添加:
services:
  backend:
    deploy:
      resources:
        limits:
          memory: 2G
```

### Q4: XRP价格显示$0.00
**问题**: 测试网上XRP价格无法获取

**解决**: 已自动修复，系统会使用mainnet API获取市场数据

---

## 监控和维护

### 日志查看
```bash
# 查看所有服务日志
docker-compose -f deploy/docker-compose.prod.yml logs -f

# 查看特定服务日志
docker-compose -f deploy/docker-compose.prod.yml logs -f backend

# 查看最近100行日志
docker-compose -f deploy/docker-compose.prod.yml logs --tail=100 backend

# 过滤关键日志
docker-compose -f deploy/docker-compose.prod.yml logs backend | grep -E '(🔄|✅|❌|⚠️)'
```

### 性能监控
```bash
# 查看容器资源使用
docker stats

# 查看特定容器资源
docker stats aicoin-backend-prod-v2
```

### 数据备份
```bash
# 备份PostgreSQL数据
docker exec aicoin-postgres-prod pg_dump -U aicoin aicoin > backup_$(date +%Y%m%d).sql

# 备份Redis数据
docker exec aicoin-redis-prod redis-cli BGSAVE

# 备份Qdrant数据
docker cp aicoin-qdrant-prod:/qdrant/storage ./qdrant_backup_$(date +%Y%m%d)
```

### 数据恢复
```bash
# 恢复PostgreSQL数据
cat backup_20250101.sql | docker exec -i aicoin-postgres-prod psql -U aicoin aicoin

# 恢复Qdrant数据
docker cp ./qdrant_backup_20250101 aicoin-qdrant-prod:/qdrant/storage
docker-compose restart qdrant
```

### 更新部署
```bash
# 拉取最新代码
git pull

# 重新构建镜像
./scripts/build_docker.sh

# 重启服务
docker-compose -f deploy/docker-compose.prod.yml up -d --build
```

### 停止服务
```bash
# 停止所有服务
docker-compose -f deploy/docker-compose.prod.yml down

# 停止并删除数据卷（谨慎！）
docker-compose -f deploy/docker-compose.prod.yml down -v
```

---

## 安全建议

1. **使用强密码**: 所有密码必须复杂且唯一
2. **定期备份**: 每天自动备份数据库
3. **监控日志**: 设置告警监控异常行为
4. **限制访问**: 使用防火墙限制端口访问
5. **更新及时**: 定期更新Docker镜像和依赖
6. **测试先行**: 所有更改先在测试网验证

---

## 技术支持

- **文档**: `docs/`目录下的所有文档
- **日志**: 查看容器日志排查问题
- **健康检查**: `/health` 和 `/api/v1/status` 端点

---

## 版本历史

- **v2.0.0** (2025-11-01)
  - ✅ 完整Docker化部署
  - ✅ 前后端分离架构
  - ✅ 实时价格更新
  - ✅ XRP价格修复（使用mainnet数据）
  - ✅ 生产环境支持

---

**祝您交易顺利！🚀**

