...
class QwenDecisionEngine:
    ...
    def __init__(self, redis_client: RedisClient, initial_capital: float = 10000.0):
        ...
        # ❌ 移除所有人工限制参数
        # self.confidence_threshold = 0.75  # 删除
        # self.max_position_size = Decimal("1000")  # 删除
        # self.risk_tolerance = 0.015  # 删除
...
"""
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a professional crypto trading AI with expertise in technical analysis and risk management. Provide accurate, objective trading decisions."
                    },
                    {
                        "role": "user",
                        "content": prompt
    ...
    def _parse_analysis_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result['model'] = 'qwen-max'
            
            logger.info(f"Qwen AI Decision: {result.get('action')} {result.get('symbol')} ${result.get('size_usd')}")
    ...
    async def _record_decision(self, decision: Dict[str, Any]):
        """记录决策"""
        try:
            decision_id = f"qwen_decision_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"
            ...
            logger.info(f"Qwen autonomous decision recorded: {decision_id}")
    ...
    async def get_decision_history(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            # 从Redis获取最近的决策
            keys = await self.redis_client.redis.keys("decision:qwen_decision_*")
